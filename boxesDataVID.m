function data = boxesDataVID( varargin )
...
dfs = {'resDir', 'vidBoxes', 'dataDir', '/disk/bingbin/VIDdevkit/', 'split', 'val'};
o=getPrmDflt(varargin,dfs,1);

% locations of PASCAL VOC dataset
if(~exist(o.dataDir,'dir')), error('dataset not found, see help'); end
imDir=[o.dataDir 'ILSVRC2015_test/Data/VID/train/'];
gtDir=[o.dataDir 'ILSVRC2015_test/Annotations/VID/train/'];
imList=[o.dataDir 'ILSVRC2015_test/ImageSets/VID/' o.split '.txt']; % i.e. VID/train.txt
addpath(genpath([o.dataDir 'VIDcode']));
...
for i=1:n, imgs{i}=[imDir ids{i} '.JPEG']; end
for i=1:n, gt{i}=[gtDir ids{i} '.xml']; end
if(~exist(imgs{1},'file')), error(strcat('images not found: ', imgs{1})); end
...
if(~exist(o.resDir,'dir')), mkdir(o.resDir); end; save(dataNm,'data');
